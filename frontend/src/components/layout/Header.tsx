...
function ConnectionIndicator({ status }: { status: ConnectionStatus }) {
    switch (status) {
        ...
        default:
            return <div className="w-2 h-2 rounded-full bg-dark-500" />;
    }
}
        ...
        <header className="h-16 bg-dark-900/80 backdrop-blur-md border-b border-dark-700 flex items-center justify-between px-6 sticky top-0 z-40">
            ...
            <div className="flex items-center gap-4">
                ...
                <div className="relative" ref={dropdownRef}>
                    ...
                    {showAgentDropdown && (
                        <div className="absolute right-0 top-full mt-2 w-80 bg-dark-800 border border-dark-600 rounded-lg shadow-xl overflow-hidden animate-fade-in z-50">
                            {/* Current Agent Info */}
                            {selectedAgent && (
                                <div className="p-3 border-b border-dark-700 bg-dark-750">
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-xs font-medium text-dark-400">Current Agent</span>
                                    ...
                                    <p className="text-sm font-medium text-dark-200">{selectedAgent.displayName}</p>
                                    <div className="mt-1 space-y-0.5">
                                        <p className="text-xs text-dark-500">
                                            <span className="text-dark-400">Project:</span> {selectedAgent.projectId}
                                        </p>
                                        <p className="text-xs text-dark-500">
                                            <span className="text-dark-400">Location:</span> {selectedAgent.location}
                                        </p>
                                        <p className="text-xs text-dark-500">
                                            <span className="text-dark-400">Agent ID:</span> {selectedAgent.id.slice(0, 12)}...
                                        </p>
                                    </div>
                                    {connectionError && (
                                        <p className="mt-2 text-xs text-danger-400 bg-danger-500/10 px-2 py-1 rounded">
                                            {connectionError}
                                        </p>
                                    )}
                                    <button
                                        ...
                                        className="mt-2 w-full flex items-center justify-center gap-2 px-3 py-1.5 text-xs font-medium bg-dark-700 hover:bg-dark-600 text-dark-300 rounded-lg transition-colors disabled:opacity-50"
                                    >
                                        {connectionStatus === 'testing' ? (
                                            <>
                                                <Loader2 className="w-3 h-3 animate-spin" />
                            ...
                            <div className="max-h-64 overflow-y-auto">
                                {agents.length === 0 ? (
                                    <div className="px-4 py-6 text-center">
                                        <Settings className="w-8 h-8 text-dark-600 mx-auto mb-2" />
                                        <p className="text-sm text-dark-400">No agents configured</p>
                                        <Link
                                            to="/settings"
                                    ...
                                    agents.map((agent) => (
                                        <button
                                            ...
                                            className={clsx(
                                                'w-full px-4 py-3 text-left hover:bg-dark-700 transition-colors flex items-center gap-3',
                                                selectedAgent?.id === agent.id && 'bg-dark-700/50'
                                            )}
                                        >
                            ...
                            <div className="p-2 border-t border-dark-700">
                                <Link
                                    to="/settings"
                                    onClick={() => setShowAgentDropdown(false)}
                                    className="flex items-center gap-2 w-full px-3 py-2 text-xs text-dark-400 hover:text-dark-200 hover:bg-dark-700 rounded-lg transition-colors"
                                >
                ...
                <div className="relative">
                    ...
                    {showUserMenu && (
                        <div
                            ref={userMenuRef}
                            className="absolute right-0 top-full mt-2 w-64 bg-dark-800 border border-dark-600 rounded-lg shadow-xl overflow-hidden animate-fade-in z-50"
                        >
                            <div className="p-3 border-b border-dark-700">
                                <p className="text-sm font-medium text-dark-200">Need Help?</p>
                                <p className="text-xs text-dark-500">Connect with me on social</p>
                            </div>
                            ...
                            <div className="p-2 space-y-1">
                                <a
                                    ...
                                    className="flex items-center gap-3 px-3 py-2 text-sm text-dark-300 hover:text-dark-100 hover:bg-dark-700 rounded-lg transition-colors"
                                >
                            ...
                            <div className="p-2 border-t border-dark-700">
                                <p className="px-3 py-2 text-xs text-dark-500">
                                    Built by <span className="text-primary-400">Yash Kavaiya</span>
                                </p>
                            </div>
                        </div>
                    )}
